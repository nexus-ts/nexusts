...
NexusJS는 `@nestjs/schedule`를 본뜬 `nexus/schedule` 모듈을 제공한다.
...
- **두 백엔드** — 인프로세스 (Bun / Node)와 Cloudflare Cron Triggers.

schedule 모듈은 **`nexus/core`와 분리**되어 있으며 자체 번들 진입점으로 제공된다.
...
import { Cron, Interval, ScheduleService } from 'nexus/schedule';
...
import { ScheduleService, scanForSchedulers } from 'nexus/schedule';
...
- **`setInterval` 드리프트** — Bun / Node의 `setInterval`은 ~10ms 정확도; 분 단위 정밀도 스케줄은 약간 드리프트할 수 있다. `maxDriftMs` 설정이 작업이 뒤처질 수 있는 한도를 제한한다.
