...
export class TracingService {
	...
	constructor() {
		// Default OTel tracer: "nexus". Even with no SDK, this returns
		// a no-op tracer that produces no-op spans — never throws.
		this.tracer = trace.getTracer("nexus", "0.4.0");
	}
	...
	async startSdk(config: import("./types.js").TracingConfig): Promise<void> {
		...
		const serviceName = config.serviceName ?? process.env.OTEL_SERVICE_NAME ?? "nexus";
