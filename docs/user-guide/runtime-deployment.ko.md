...
NexusTS는 단일 `Application` API를 통해 **Bun과 Cloudflare Workers**를 대상으로 합니다. 프레임워크는 런타임을 자동 감지하고 적절한 어댑터를 로드합니다.
...
- **`emitDecoratorMetadata`는 Cloudflare의 esbuild가 무시합니다** — 항상 명시적 `@Inject(Token)` 또는 필드 인젝션(`@Inject(Token) declare field`)을 사용하세요.
...
멀티 타겟 빌드(Bun + Workers)의 경우 `entrypoints`에 항목을 추가:
