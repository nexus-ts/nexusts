...
`nexus/openapi` is the **default OpenAPI generator** for NexusJS. It
...
`nexus/openapi` ships a Scalar HTML page that loads the JS
...
| --- | --- | --- |
| NestJS | `@nestjs/swagger` (Zod support via `@anatine/zod-nestjs`) | ✅ closed — `nexus/openapi` |
| AdonisJS | Community packages (`adonis-autodoc`) | ✅ closed — `nexus/openapi` |
