...
import type { AuthUser, AuthVariables, AuthConfig } from '../../src/auth/types.js';
...
describe('authMiddleware', () => {
	...
	it('skips ignored paths entirely', async () => {
		const auth = createAuth(AUTH_CONFIG);
		const app = new Hono<{ Variables: AuthVariables }>();
		app.use('*', authMiddleware(auth, { mode: 'required', ignoredPaths: /^\/health$/ }));
...
describe('CurrentUser decorator', () => {
	...
	it('accepts all option shapes without throwing', () => {
		...
		expect(() =>
			CurrentUser({ assert: (_u: AuthUser) => true }),
		).not.toThrow();
	});
});