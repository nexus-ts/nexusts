...
`nexus/sse`는 서버에서 클라이언트로의 단방향 스트리밍을 위한 Hono
...
| 필요 | 사용 |
| --- | --- |
| 단방향 서버 → 클라이언트, 순서 보장, 저빈도 | `nexus/sse` |
...
| 프레임워크 | SSE | v0.4 |
| --- | --- | --- |
| NestJS | `@nestjs/platform-sse` (RxJS observable 사용) | ✅ 해소 — `nexus/sse` |
| AdonisJS | DIY (문서의 Hono `streamSSE` 예제) | ✅ 해소 — `nexus/sse` |

v0.3 격차 분석(NestJS §3.3, AdonisJS §4.4)에 따르면, 이것이 Tier 2
격차였다. `nexus/sse`는 이를 1급 모듈로 출시한다.
